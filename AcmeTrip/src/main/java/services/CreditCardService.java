...
public class CreditCardService {
	...
	public void delete(CreditCard ccard) {
		checkPrincipal(ccard.getManager());
	...
	public Collection<CreditCard> findCreditCardByPrincipal() {
		Manager manager = managerService.findByPrincipal();
		Collection<CreditCard> result = creditCardRepository
				.findCreditCardsByManager(manager.getId());
		Assert.notNull(result);
		return result;
	}
	...
	// barter.setRegistrationMoment(new Date(System.currentTimeMillis() -
	// 1000));
